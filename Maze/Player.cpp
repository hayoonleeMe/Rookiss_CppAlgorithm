...
void Player::AStar()
{
	...
	vector<vector<int>> best(size, vector<int32>(size, INT32_MAX));
	...
	while (pq.empty() == false)
	{
		...
		// 동일한 좌표를 여러 경로로 찾아서, 더 빠른 경로로 인해서 이미 방문(closed)된 경우 해당 좌표에 대한 로직 스킵
		...
		// 방문
		// 이미 방문했으면 이 경우보다 더 좋은 경우는 생기지 않는다.
